$summon item_display ~ ~1 ~ {item:{id:item_frame,components:{item_model:"$(magick_circle)"}}, Tags:["magick_circle"],brightness:{sky:10,block:10},transformation:{scale:[0f,0f,0f],left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f]}}

$data merge entity @e[type=item_display,tag=magick_circle,sort=nearest,limit=1] {data:{duration: $(duration),scale:$(scale)}}
...
data modify entity @e[type=item_display,tag=magick_circle,sort=nearest,limit=1] Rotation[0] set from entity @s Rotation[0]
data modify entity @e[type=interaction,tag=magick_circle,sort=nearest,limit=1] Rotation[0] set from entity @s Rotation[0]
