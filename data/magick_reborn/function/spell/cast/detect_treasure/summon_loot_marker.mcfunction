...
execute as @e[type=magma_cube,tag=treasure_marker,sort=nearest,limit=1] unless block ~ ~ ~ chest run attribute @s scale modifier add smaller -0.04 add_multiplied_base
execute as @e[type=magma_cube,tag=treasure_marker,sort=nearest,limit=1] if block ~ ~ ~ chest run attribute @s scale modifier add smaller -0.16 add_multiplied_base
