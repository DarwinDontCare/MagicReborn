...
$data modify storage magick:data magick_circle set value {magick_circle: "magick_reborn:destruction_magick_circle", duration: 30, scale: 5, caster_id: $(uuid), is_burst: 0, effects: [{effect: "minecraft:meteor"}]}
...
$summon armor_stand ~ ~ ~ {Tags:["meteor_ride"],data:{caster_id:$(uuid), amplifier: $(amplifier)},Invisible:true,Invulnerable:true}
...
execute as @e[type=armor_stand,tag=meteor_ride,distance=..3,sort=nearest,limit=1] run scoreboard players set @s ticksCount 4

execute as @e[type=item_display,tag=meteor,distance=..3,sort=nearest,limit=1] run data merge entity @s {start_interpolation:0,interpolation_duration:4,transformation:{right_rotation:[360f,360f,0f,1f],scale:[3f,3f,3f]}}
...
execute as @e[type=armor_stand,tag=meteor_ride,distance=..3,sort=nearest,limit=1] store result entity @s Motion[0] double .00005 run scoreboard players get @s MotionX
execute as @e[type=armor_stand,tag=meteor_ride,distance=..3,sort=nearest,limit=1] store result entity @s Motion[1] double .00005 run scoreboard players get @s MotionY
execute as @e[type=armor_stand,tag=meteor_ride,distance=..3,sort=nearest,limit=1] store result entity @s Motion[2] double .00005 run scoreboard players get @s MotionZ
