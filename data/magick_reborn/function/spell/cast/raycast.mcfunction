...
$execute positioned ^ ^ ^$(index) if score #Raycast boolean matches 1 as @e[predicate=!magick_reborn:ignore_entity, distance=..1,limit=1] unless score @s uuid matches $(uuid) at @s run return run summon marker ~ ~ ~ {NoGravity:true,Invulnerable:true,Tags:["raycast_hit_$(tag)"],data:{caster_id: $(uuid)}}
