...
$execute at @e[tag=raycast_hit_tk,nbt={data:{caster_id: $(uuid)}},limit=1] as @e[predicate=!magick_reborn:ignore_entity,distance=..2,limit=1,tag=!raycast_hit_tk,tag=!tk_target] run return run function magick_reborn:spell/cast/telekinesis/tk_entity {uuid: $(uuid), amplifier: $(amplifier)}