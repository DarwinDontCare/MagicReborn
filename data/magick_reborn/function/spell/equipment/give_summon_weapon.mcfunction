...
$execute if score #Compare boolean matches 0 unless entity @s[nbt={Inventory:[{components:{"minecraft:custom_data":{summon_type: "$(tool)"}}}]}] run give @s $(tool)[custom_name=$(name), enchantment_glint_override=true,item_model="$(model)",enchantments={binding_curse:1,vanishing_curse:1,unbreaking:5,infinity:1},custom_data={summoned:true, summon_type: "$(tool)"},max_damage=$(duration),attribute_modifiers=[{type:attack_damage,amount:$(amplifier),operation:add_value,id:"1748357971244"}]]

$execute unless score #Compare boolean matches 0 unless entity @s[nbt={Inventory:[{components:{"minecraft:custom_data":{summon_type: "$(tool)"}}}]}] run give @s $(tool)[custom_name=$(name), enchantment_glint_override=true,item_model="$(model)",enchantments={binding_curse:1,vanishing_curse:1,unbreaking:5},custom_data={summoned:true, summon_type: "$(tool)"},max_damage=$(duration),attribute_modifiers=[{type:attack_damage,amount:$(amplifier),operation:add_value,id:"1748357971244"}]]
