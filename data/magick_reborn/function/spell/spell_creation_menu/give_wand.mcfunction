...
$say cost $(magick_cost)

$execute unless data storage magick $(uuid).known_spells run data modify storage magick $(uuid).known_spells set value []
$data modify storage magick $(uuid).known_spells append value {spell_name: "$(spell_name)", custom_data: {cost: $(magick_cost), effects: $(spell_effects), type: "wand", enchanted: true, tier: $(tier), model: "$(model)"}, lore: [[{"text":"$(effect0)","italic":false}],[{"text":"$(effect1)","italic":false}],[{"text":"$(effect2)","italic":false}],\
[{"text":"$(effect3)","italic":false}],[{"text":"Cost: $(magick_cost)","italic":false}]]}
