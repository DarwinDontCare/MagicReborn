...
,custom_data={cost: $(magick_cost), effects: $(spell_effects), type: "wand", enchanted: true, tier: $(tier), model: "$(model)", id: $(id)},\
...
$data modify storage magick $(uuid).known_spells append value {\
    ...
    lore: [\
        [{"text":"$(effect0)","italic":false}],\
        [{"text":"$(effect1)","italic":false}],\
        [{"text":"$(effect2)","italic":false}],\
        [{"text":"$(effect3)","italic":false}],\
